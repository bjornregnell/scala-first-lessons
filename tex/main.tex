...
\end{table}
  \begin{itemize}
    ...
    \item Computer rooms are available to students 24/7.
  \end{itemize}

\vfill Link to schedule: \url{http://cs.lth.se/pgk/schema/}
\end{Slide}  

\begin{Slide}{The first week is special}
  
  \begin{itemize}
    \item Simple intro to programming is using REPL and Kojo with turtle graphics%: \url{http://www.kogics.net/kojo-download}: \\ Working with expressions in REPL and control flow principles (SARA) in Kojo: 
    \begin{itemize}
      ...
      \item \textbf{A}bstraction (simple procedures)   
    \end{itemize}
    \item Computing course in parallel with intro to terminal on school computers (Ubuntu with preinstalled Scala tools)
    \item Student's playful freshman activities in parallell
